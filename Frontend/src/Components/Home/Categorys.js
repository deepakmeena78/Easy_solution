    ...
    const fetchCategories = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3200/category/category-get"
        );
    ...
    <>
      ...
      <div className="w-full mx-auto p-6 text-center overflow-hidden bg-white">
            ...
            categories.map((data, index) => (
              ...
              >
                <img
                  ...
                  className="w-24 h-24 object-contain mx-auto rounded-md"
                />
                <h3 className="mt-2 text-lg font-semibold">{data.category}</h3>
        ...
        <button
          ...
          className="mt-6 px-6 py-2 bg-[var(--dark-blue)] text-white rounded-lg hover:bg-[#23474C] m-6"
        >
          VIEW ALL CATEGORIES
        </button>
      </div>
